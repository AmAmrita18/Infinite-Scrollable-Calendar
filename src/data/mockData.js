...
// Process journal entries and convert dates
export const hairJournalData = rawHairJournalData.map((entry, index) => ({
  ...entry,
  id: index + 1,
  parsedDate: dateUtils.parse(entry.date, 'dd/MM/yyyy')
}));

// Helper function to get entries for a specific date
export const getEntriesForDate = (date) => {
  return hairJournalData.filter(entry => 
    dateUtils.isSameDay(entry.parsedDate, date)
  );
};